...
Next, we can load the example dataset from the package using the `data()` function. It is a tibble that contains 45 columns numbered `bhitop01` to `bhitop45` containing each participant's rating on each item of the B-HiTOP (on a numerical scale from 1 to 4).
