...
Which columns it recognizes depends on `prefix`, which must match how your item columns are actually named. The default, `"hbr_"`, is how both example datasets and the package's own exports name them (`hbr_01` to `hbr_45`), so `sim_hitopbr` needs no `prefix`; data named some other way, such as `HBR_01`, would pass `prefix = "HBR_"`:
