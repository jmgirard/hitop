...
rank_scales(facet_scores, scales = 1:25, prefix = "pid_", top = 5)
```

The added `top_scales` column holds one comma-separated string per participant: the names of their five highest-scoring facets, in descending order. Ties are broken by the order of the columns you selected. Use `name` to call the output column something else, `dir = "low"` to rank from the bottom instead, and `append = FALSE` to get back the ranked column on its own.
