...
Every number returned is a cell of a published table: the nearest printed row is selected and nothing is interpolated. A score that falls outside a printed range is capped to the nearest end rather than extrapolated, and a message reports how many observations that happened to — the `PRD` sum reaches 66 while its table stops at 55, so it is a common one to see. A scale the tables do not cover returns `NA` in both columns with a message naming it.
