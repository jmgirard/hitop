...
asks for the identifier instead.
...
“Completion URL” field. After a confirmed send the page sends the
...
confirmed, the page shows the file name first and then a link to that
address, so the participant sees the file’s name before leaving.
