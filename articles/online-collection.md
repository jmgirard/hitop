...
instrument’s order rather than the order shown. The script builds the
header from the first row it receives and appends a key it has not seen,
so a sheet that already holds rows without `item_order` puts that column
