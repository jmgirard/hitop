...
default layout, as the first scoring call in this article does.) Name
the item columns by position here rather than by name: under
`layout = "printed"` the names carry item numbers out of instrument
order, and naming them would trip the check that warns of a misordered
mapping.
