...
Some browsers save the download as a plain text file
(`hitophsum_qualtrics.txt` or `hitophsum_qualtrics.qsf.txt`) because the
file is served as text. That is only the file name: renaming it back to
`hitophsum_qualtrics.qsf` is safe and enough for Qualtrics to accept it.
If your browser displays the file instead of downloading it, use **Save
link as…** (right-click or Control-click the download link) rather than
copying the text out of the browser window.
