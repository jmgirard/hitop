...
The download buttons on the instrument pages save the file under its own
name. If you obtain the `.qsf` some other way — from the GitHub
repository, say — your browser may name it `.txt`, because GitHub serves
the file as plain text. That is only the file name: renaming it back to
`.qsf` is safe and is all Qualtrics needs. If the browser displays the
file instead of downloading it, use **Save link as…** (right-click or
Control-click the link) rather than copying the text out of the browser
window.
