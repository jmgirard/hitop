...
Report (HiTOP-BR).* Manuscript submitted to *Assessment*, Manuscript ID
`ASMNT-26-0390`. Shelf copy: `cairn/references/sources/ASMNT-26-0390_Proof_hi.pdf`
(gitignored; sha256 `1c211219b7fe13f8ed172f9210c152a642a9be77d790e08d795843c25da8e425`).
The scale names read from it come from **Table 1** ("Descriptive Statistics and
Internal Consistencies for all HiTOP-SR Primary Scales, Subscales, and ...", p. 49
of the shelf PDF), the paper's enumerative table over every primary scale and
subscale. This is the naming authority for a HiTOP-SR scale under D-018's
per-content-type rule: the development workbook `HiTOP-SR-Final.xlsx` is primary
for item-to-scale membership, reverse flags and item text — the content the keying
was built from — and prints no full name for the one scale it abbreviates.

**Status of the source.** The document is a manuscript under peer review, not an
...
2023-12-01; shelf copy `cairn/references/sources/simms2023_hitop_measurement_workgroup.pdf`,
sha256 `903e277334924d42286bbf261857ddea539b87e00eff5563784686711614681a`), slide 34,
prints the same scale with the same item count (6) and the same alpha (0.83) that
Table 1 gives, so the deck's table and Table 1 are the same content at two dates.
It is corroboration for the scale's identity, not for its spelling — see OQ-3.

**Not corroboration.** `hitopsr_definitions` is *not* evidence for any HiTOP-SR
scale name, in either direction. Those definitions are the maintainer's own text,
written for the Society and supplied to the paper's authors, where they were
published as the paper's **Appendix A** ("Definitions and Client-facing Descriptions
for HiTOP-SR primary scales and subscales"). The appendix therefore descends from
...
this one scale. Inventory regenerated by `data-raw/verify_hitopsr_scale_name.R` on
2026-08-27 against the shelf PDF, page numbers being the shelf PDF's own:

```
Non-suicidal Self-injury   3 occurrences on pages 49, 52, 55
Non-Suicidal Self-injury   1 occurrence on page 23
Non-Suicidal Self-Injury   1 occurrence on page 18
[wrapped occurrence]       page 69 (name wraps across a table cell; read by eye)
```

Pages 49, 52 and 55 are Tables 1, 2 and 3; pages 18 and 23 are body prose, the
first being the sentence that introduces the `NSSI` abbreviation. Page 69 is
Appendix A, whose cell wraps mid-name and whose text is the maintainer's own (see
the section above), so it is neither counted nor treated as evidence. The paper
Title-Cases every other multi-word scale name — checked 2026-08-27 across six
probes (`Social Aloofness` 5, `Anxious Worry` 5, `Angry Hostility` 4, `Manic Energy`
9, `Restricted Affectivity` 4, `Trauma Reactions` 6; every lowercase variant 0) —
so the tables' lowercase `injury` is idiosyncratic to this scale rather than a house
style.

A fourth rendering sits outside the paper: the December 2023 Workgroup deck's slide
34 prints `Non-suicidal Self-Injury (NSSI)`, agreeing with the tables on `suicidal`
and with the prose on `Injury`.

**What the package adopted, and why.** `Non-suicidal Self-injury`, the tables'
rendering, because the tables are the paper's enumerative naming surface — the
place it lists every scale, which is the content type `hitopsr_items$Scale` holds —
where the prose mentions the scale in passing. Signed off in
[D-041](DECISIONS.md).

**Resolution.** The accepted version's Table 1. If it prints a different casing the
adopted name follows it, which costs a rebuild of the two Word artifacts and an
amendment here; if it dehyphenates to `Nonsuicidal`, the derived column stem changes
too and the decision reopens. Worth putting to the authors, since a paper that
renders one of its 76 scale names three ways is likelier to have a copyediting
inconsistency than a naming intention.
