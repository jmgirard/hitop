...
- [x] AC5: `devtools::check()` returns 0 errors, 0 warnings and 0 notes, `pkgdown::check_pkgdown()` is clean, `pkgdown::build_site()` renders the six download pages and the overview page without error, and `NEWS.md` carries an entry naming the strip and the step list.
...
- AC5: `devtools::check()` 0 errors, 0 warnings, 0 notes; `pkgdown::check_pkgdown()` "No problems found"; `pkgdown::build_site()` finished, the six download pages and the overview rendered (built pages: 3 cards each, one `.hitop-online` on five and none on HSUM, the overview `<ol class="hitop-steps">` present); `NEWS.md` lines 5 to 15 name the strip and the four steps. ✔

**Consistency gate.** `cairn_validate.py` all checks passed (24 advisories, none new). `devtools::document()` produces no diff. README.Rmd and README.md untouched by the branch. `check_pkgdown()` clean. NEWS entry present under the development version. No new top-level file (DESCRIPTION, NEWS.md and `_pkgdown.yml` pre-exist in `.Rbuildignore` terms). Full `check()` 0/0/0. No principle changed, `cairn_impact` not owed.

**Independent review** (user-facing tier, three lenses, fresh contexts). [S] prior-review lens: no prior-review evidence contradicted; PR-comment probe returned nothing; 0 findings. [S] blame-history lens: 0 findings; one cosmetic observation, the `hitop-steps` class has no CSS rule (same as F11 below). [O] diff-bug lens: 15 findings, ranked by the lens, verified here where they rest on behavior:

- F1 D-074 says the R CMD check matrix enforces the menu test; under check the test skips (`_pkgdown.yml` is build-ignored and `vignettes/articles` is not under `hitop.Rcheck/`), so the CI job that runs it is test-coverage, whose covr run reads the source tree. Verified from `.Rbuildignore` and the workflow. Proposed: fix now, reword the entry's Consequences before it reaches main.
- F2 The strip paragraph's "that page" has no antecedent, "they fill with `read_form_responses()`" misreads, and the removed JSON card's description of the file is gone. Proposed: fix now, rewrite the paragraph.
- F3 The overview link test drops a `#fragment`, so a renamed tutorial heading breaks step 3's link unnoticed. Proposed: fix now, resolve fragments against the target's heading ids.
- F4 `online_strip()` renders a mismatched `stem` and `json_link` without error (verified: the PID-5 link beside the PID-5-SF JSON). Proposed: fix now, `stop()` on a mismatch, with a test.
- F5 The strip's `<h5>` sits under an `<h3>`. Proposed: fix now, `<h4 class="h5">`.
- F6 The emoji is read aloud in the heading. Proposed: fix now, `aria-hidden` span.
- F7 The strip is a plain `div` with no landmark. Proposed: fix now, `<section aria-labelledby>`.
- F8 The "Make a study link" button leaves the site with no cue. Proposed: reject, the site's other external links (the two Instruments menu entries) carry none; a cue is a site-wide convention.
- F9 The tutorial sentence documents a `c` carrying a module before any page produces one, and its lines exceed the wrap. Proposed: fix now for the wrap; reject the content part, the plan's scope asked for that sentence and link.html's prefill is what it describes.
- F10 Step 3's link text "sends each participant's responses" targets "Make the study link". Proposed: reject, that section's second paragraph is the one describing the send.
- F11 `hitop-steps` has no CSS rule. Proposed: reject, the class is the test's selector hook, as `.hitop-downloads` is the layout's.
- F12 `.hitop-online`'s `background-color: var(--bs-body-bg)` repeats the page background. Proposed: reject, it matches `details.hitop-versions`.
- F13 `skip_if_not_installed("jsonlite")` never fires (Imports). Proposed: fix now, remove.
- F14 The download-attribute regex leaves `.` unescaped. Proposed: reject, pre-existing pattern the diff reuses.
- F15 `\U1F310` can print as `<U+1F310>` on pre-4.2 Windows R. Proposed: fix now, the literal emoji as the cards use.

**Gate triage (2026-09-25, Jeff):** the proposed dispositions accepted; ten fixed now, five rejected with the reasons above. Fixes on the branch: D-074's Consequences reworded before the entry reaches main (F1); the strip is a `<section aria-labelledby>` with an `<h4 class="h5">` whose emoji is `aria-hidden` and literal, and a four-sentence paragraph naming the hitop-form page, `read_form_responses()` and what the JSON export holds (F2, F5, F6, F7, F15); `online_strip()` stops on a `stem` that does not match its JSON link, with a test (F4); the overview link test resolves a `#fragment` against the target article's pandoc heading ids (F3); the jsonlite skip removed (F13); the tutorial sentence re-wrapped (F9). Plants: a renamed fragment and a removed stem guard each turn one test red. `test-download-pages.R` 92, `test-artifacts.R` 137, `test-overview.R` 9 pass.

**Re-verification after the fixes (3d945206):** full suite FAIL 0 with the 15 merge-base skips; `check()` 0 errors, 0 warnings, 0 notes; `check_pkgdown()` clean; `build_site()` finished, the six built download pages hold 3 cards each, a `<section class="hitop-online" aria-labelledby>` on the five form-backed pages and none on HSUM. AC1 to AC5 stand on this head.
