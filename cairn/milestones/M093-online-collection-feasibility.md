...
- 2026-09-20: step-7 approval: m093-online-collection-feasibility approved for merge (re-posed after the CI pin; comment fixes and the revert candidate row applied)
...
- Second review pass (2026-09-20), executable surface: the CI pin in `.github/workflows/R-CMD-check.yaml`, three fresh-context lenses.
  ...
  - Gate triage 2026-09-20 (second pass): C1, C3, C4 fixed in the workflow comment; C2 and the untested rspm alternative carried by the new ROADMAP candidate row "Revert the macOS CI pin"; C5 noted. To fit the row, M090's done row was pruned early (retention is an upper bound) and the builder smoke-test row compressed.
