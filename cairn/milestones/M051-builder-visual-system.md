...
- [ ] AC5: The privacy notice, in whatever wording it now carries, still states that the page builds blank questionnaires and scores nothing, that it never receives anyone's responses, and that nothing selected or built leaves the page; and it names every host the page fetches from, `repo.r-wasm.org` among them. Over one full page load plus one build of each of the three formats, every network request the browser records goes to the page's own origin or to a host the notice names.
...
- [ ] T1: Capture the AC1 baseline from the deployed page — each matrix cell's file, plus the reported `hitop` version — by patching `URL.createObjectURL` to keep the blob and returning early from the anchor's click, driving the controls through `read_page` refs and `form_input` (never screenshot coordinates). Record the current control-group titles, live-region arrangement and the four crosswalk sentences alongside.
- [ ] T2: Author the visual system in the page's `<style>` block: type scale, spacing scale, colour tokens for both schemes, surface and border treatment, button/input/checkbox states, and a focus ring meeting AC2's non-zero-width, non-transparent bar.
- [ ] T3: Apply the system to the existing markup without moving elements — the notice, the scale grid, the four control groups, the download row, the status line and the log pane — and make each fluid at 360px.
- [ ] T4: Rewrite the page's copy — headings, labels, group titles, hints, the privacy notice (adding `repo.r-wasm.org`), the shuffle notice and its three crosswalk sentences, the status and log strings — keeping every factual claim each one makes.
