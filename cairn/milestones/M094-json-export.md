...
- [ ] AC2: Each of the two files has a `hitop_artifacts` row with `format` `"json"` whose `md5` equals the committed file's, and a byte-identical copy under `pkgdown/assets/downloads/`.
...
- [ ] T3: `tests/testthat/test-json-export.R`: the AC1 and AC3 assertions with the expected side from the tables; before trusting green, plant in a temporary copy, one at a time, a changed item text, a dropped item, two swapped items, a changed option label and a changed `stem`, and see the test red on each.
- [ ] T4: Download articles and NEWS; run `data-raw/check_line_endings.R`; `devtools::document()`; `devtools::test()`; `devtools::check()`.
