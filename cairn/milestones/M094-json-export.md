...
- AC5 evidence (pass 2, after the gate fix G2): card text is now "Use this machine-readable file, which a web form outside the package can read, to present the instrument's items, response options and instructions as the package's tables hold them." — one sentence, imperative opener, the three contents named, "can read" stated; `devtools::test()` 0 failures, 17,399 passes on this head; `devtools::check()` 0/0/0 ran on the head before this fix, which changed one string in a `vignettes/articles/` file outside the built package, a DESIGN.md sentence and an argument name in `data-raw/`.
- Independent review, pass 2, three lenses, 2026-09-20. [S-blame]: no conflict with main or any D-entry; noted that the json manifest rows' `changes` text was rewritten between T2 and T5 inside the branch (never on main), which the squash collapses — noted, no action. [S-prior]: no regression against archived reviews; all seven pass-1 fix-now items confirmed. [O]: all seven fixes verified against the implementation (script mode byte-identical; manifest reproducible from main's 39 rows through `artifacts.R`); eight new findings:
  - G1 — the export is a new public file-format contract (`format` `"1.0"`) with no D-entry, where D-039(d) put the module descriptor's fields under D-entry governance. Posed at the gate: D-entry at hygiene.
  - G2 — the card said "drive a web form" and never "can read it" as AC5 requires. Fixed now: card reworded (above).
  - G3 — the DESIGN.md generator-list insertion sat before "The HSUM Qualtrics artifact is the exception: no exported generator", which it falsified. Fixed now: the JSON sentence moved after the HSUM sentence and states "likewise with no exported generator".
  - G4 — `source("data-raw/json_export.R")` from a console without the package loaded still fails at `json_specs`. Rejected: the header documents two entry points (sourced by `artifacts.R`, which loads the package, or run as a script), both work.
  - G5 — writer and test both read `utils::packageVersion("hitop")`, so nothing anchors to the DESCRIPTION file AC3 names. Rejected: `packageVersion()` reads the package's DESCRIPTION; under `load_all()` and `check()` that is the source file.
  - G6 — `export_report()` checks named fields only, so an extra top-level or per-object key ships green. Follow-up: joins the test-quality candidate row with F7–F9, F16.
  - G7 — `air format --check` would reformat the two new files. Rejected: formatter territory; `artifacts.R` was already unformatted on main.
  - G8 — the writer's argument `file` shadowed `file()`. Fixed now: renamed `path`.
- Pass-1 findings still open, triaged at the gate: F2 (stem-named items `hitopsr_001` versus the package prefix `hsr_001`) — follow-up, recorded for M095/M096 planning in the online-form candidate row; F3 (a version bump reds the suite until `artifacts.R` reruns) — follow-up, one line in the release-walk candidate for the next `/cairn-release`; F6 (`Sys.Date()` churn on rerun) — follow-up with F3; F7, F8, F9, F10, F16, G6 — one test-quality candidate row; F14 (four cards wrap) — rejected, cosmetic; F17 + G1 (no discriminator from the module descriptor, no D-entry) — gate question.
- 2026-09-20: gate (Jeff): G1/F17 → a D-entry at hygiene declaring format 1.0's fields a public contract, the discriminator deferred to the online-form candidate row for M096; all other dispositions above accepted as triaged.
- 2026-09-20: step-7 approval: m094-json-export approved for merge
