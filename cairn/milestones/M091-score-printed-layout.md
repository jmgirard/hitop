...
Review run 2026-09-11 on branch head 788bc108, 6 commits past main. Main equals origin/main and did not move, so no merge was needed.

- AC1: verified. `data-raw/characterize_layout/characterize.R` was rerun from a fresh scratch tree. Grep enumerates 93 call sites in the merge-base test files. Each run recorded 251 calls (132 `score_hitopsr()`, 119 `reliability_hitopsr()`, 15 errors) at merge base c54ea081 and at the branch head. `compare.R` reports 251 of 251 `identical()`.
- AC2: verified. `test-layout.R` passes in the full suite. The keyed pair (HSR 310, hand-written expected means), the unkeyed pair, the four-scale module under a 3-cycle and a seeded shuffle, and the whole-instrument module all pass. The fixture helper asserts `!identical(match(item_order, module$items), match(module$items, item_order))`. The diff-bug reviewer recomputed the eight hand-fixture values and confirmed them. A spot-check in this session on a 5-cycle Romantic Disinterest module returned scores identical to the instrument-order call, and different scores under the default layout.
- AC3: verified. The alpha test and the mocked `calc_omega` matrix-capture test pass in the full suite. The captured matrix equals the one the instrument-order call passes.
- AC4: verified. The three refusal tests and the bad-value test pass. Each asserts the argument name and the blamed call. Spot-check in this session: `layout = "nope"` aborts from `score_hitopsr()` with `` `layout` must be one of "instrument" or "printed", not "nope" ``. The refusals are plain `cli_assert()` aborts with no class.
- AC5: verified. Both warning-count tests pass on both functions. Names `q_1` to `q_n` warn zero times. Non-ascending names warn exactly once.
- AC6: verified. The grep over `R/`, `vignettes/`, and `README.Rmd` returns zero hits (exit 1). The passages in the roxygen of `generate_docx_hitopsr()`, `read_module()`, and `write_module()` and in the modules article (lines 127 to 137 and 320 to 336) were read. Each names `layout = "printed"`. The two scoring-function sources name it five times each.
- AC7: verified. `devtools::document()` leaves no diff. `devtools::test()` reports 17383 pass, 0 fail, 0 warn, 13 skip. `devtools::check()` reports 0 errors, 0 warnings, 0 notes.

Consistency gate: `cairn_validate.py` exits 0 with all checks passing. Its 24 advisory warnings are pre-existing (dangling D-id tokens and one references-staleness note). No principle changed, so `cairn_impact.py` was skipped. Toolchain: document() no diff. README.Rmd is untouched and not newer than README.md. `pkgdown::check_pkgdown()` reports no problems. NEWS.md carries the `layout` entry with no milestone number. The new `data-raw/characterize_layout/` sits under the existing `^data-raw$` Rbuildignore entry. check() is clean. Driving RR: none.

Independent review, three fresh-context lenses:
- [S] blame-history: no history conflict. One informational finding: `rlang::arg_match()` for `layout` sits beside the family's `match.arg()` convention (commit 0c33f8ee moved only the `dir` argument of `rank_scales()` to `arg_match()`), and the rationale lives in code comments only.
- [S] prior-review-record: no regression of a past-review lesson. The PR-comment probe returned an empty array, so the walk was skipped.
- [O] diff-bug: no correctness bug. The permutation direction was verified in R for names and positions and on the `calc_se` path. Findings, most severe first:
  1. The modules article's copy-paste snippet (`modules-hitopsr.Rmd:331`) passes `items = names(collected)`. That fires the ascending-name warning for printed-order `hsr_` names, which is why both roxygen examples were switched to positions.
  2. `layout` is inserted before the deprecated `subset` in both signatures, so a caller passing `subset` positionally now binds it to `layout`.
  3. A non-integer `item_order` such as `42.4` passes the permutation check through `as.integer()` truncation. The result is still correct.
  4. The permutation check duplicates the one in `write_module_impl()` verbatim.
  5. Under `"printed"`, item validation runs before data validation, so a bad-data call can report the items error first.
  6. `match.arg(missing)` and `rlang::arg_match(layout)` sit side by side in `score_hitopsr()`.
  7. The characterization scripts sit in `data-raw/` though they generate no data, and `compare.R` reads bare `base.rds` and `head.rds` from the working directory.
