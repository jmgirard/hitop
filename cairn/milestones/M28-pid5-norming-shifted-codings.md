...
- [ ] **T1.** Replace M27's `srange` guard: the option-count check, the all-`NA` path, and
      the `cli::cli_warn()` report.
- [ ] **T2.** Implement the two per-metric rescale branches and assert the three no-ops.
- [ ] **T3.** Tests per AC5 — the count branch, two adjusted-metric fixtures, three
