...
- [ ] T1: Add the three PID-5 entries to `INSTRUMENTS` (`form.js:14`) and the three `<option>`s to the selector (`link.html:67-70`). If the module hint (`link.html:81`) reads wrong with five instruments, reword it without widening what a module applies to.
...
- [ ] T5: Confirm the deployed site serves the three PID-5 files. Open the PR and get the `tests.yml` run green. After the merge, dispatch one run against the deployed page. Record both runs in the work log.
