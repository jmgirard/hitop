...
- 2026-09-20: gate (Jeff): the proposed triage split accepted. Seven hardening fixes landed in the reader with tests, finding 1 to the printed-layout candidate row, five rejected with reasons (Review section).
- 2026-09-20: step-7 approval: m096-form-responses-reader approved for merge.
...
Triage (Jeff, 2026-09-20 gate, the proposed split accepted). Fixed now on the branch, each with a test. Finding 2: an out-of-range value is refused as outside the integer range, with no base warning. Finding 3: fractional seconds parse through `%OS`. Finding 4: a duplicated column is refused naming the file and column. Finding 5: both stamps are matched whole before parsing. Finding 6: one message line per differing file with its own reason, tested through `cnd$body`. Finding 8: `.CSV` is scanned too. From 13: the trailing-newline warning is muffled and a folder named `.csv` is skipped. Follow-up: finding 1 extends the existing printed-layout candidate row in ROADMAP with the wrong-scores fact. Rejected: 7, because the page never writes a literal `NA` and the refusal names the column. Rejected: 9, the article path was Jeff's implement-gate decision. Rejected: 10, positions select the item columns and the criterion names which columns, not how. Rejected: 11, resolved before the gate. Rejected: 12, standing drift the diff did not introduce. Not actioned from 13: the zero-item file and the loose `expect_error` pattern, both harmless. After the fixes, the reader's test file runs 29 tests and 89 expectations green, and the full suite is green.
