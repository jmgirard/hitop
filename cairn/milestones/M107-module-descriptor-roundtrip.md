...
- 2026-09-22: T3 done. `write_module_impl()` rebuilds the module with `hitop_module()` before it opens the path, refuses `items` or `nItems` that differ from the rebuild, and writes the rebuild's fields. Plants: 5 location and 4 form plants on `items`, 5 on `nItems`, and 2 that fail the rebuild (an unknown scale and the instrument `"pid5"`). All 3 refusal tests were red before the fix, and the double-items control passed. `devtools::test()`: 0 failed, 13 skipped, 720 tests.
