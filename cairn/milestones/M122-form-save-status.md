...
- Gate, r-package: `devtools::document()` produced no diff. README.Rmd untouched on the branch and README.md newer than it, so in sync. `pkgdown::check_pkgdown()`: no problems found. NEWS.md carries the entry (AC4). No new top-level file. The branch touches no R code, so `NAMESPACE`, `man/` and `data/` are unchanged.
