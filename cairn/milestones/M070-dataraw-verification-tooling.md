...
- [ ] T1 Delete the three spent one-shot verifiers; sweep the tracked tree with `git grep -n -F` for each basename and confirm the only survivors are the archive summaries, which are history and stay untouched.
