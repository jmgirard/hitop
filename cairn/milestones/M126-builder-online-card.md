...
- [ ] AC4: `package.json` gains `"prose": "node tests/prose.mjs"`, `smoke.yml` runs `npm run prose` on every push and pull request, and that run exits 0 with every new visitor-facing string classified in the ledger. `tests/plants.mjs` gains one plant per new smoke assertion: the online card absent, the anchor's `c` carrying a module that differs from the saved file, the anchor's `c` lacking `instrument`, the anchor missing `target` or `rel`, and an anchor left in the document after a Word build. Every plant in the matrix is caught by at least one assertion.
