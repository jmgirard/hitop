...
- 2026-08-26: triage at the gate, as directed — findings 1, 2, 3 and 5 fixed on the branch; findings 4 and 6 deferred onto the standing "Generalize modularization to BR/PID-5" candidate row (search-first: that row already owns the second-instrument case both findings need); findings 7-10 rejected with the reasons recorded above.
- Fix for finding 1: the article's prose now names the fourth column and says the print truncates it, written against the observed print ("Fear and avoidance of situation…").
- Fix for findings 2 and 3: `scale_definitions()` gains `call = rlang::caller_env()` and forwards it, and the `Brief` column is resolved before the `tibble()` call rather than inside it — a promise forced in `tibble()`'s frame left the abort with no call at all, which the first attempt at this fix showed. The condition now carries `available_scales("hitopsr")`, matching what the neighbouring `hitop_unsupported_instrument` guard carries. The hint no longer names the `.Rbuildignore`d `data-raw/` script; it names the issue tracker. A new test pins both, red (340 pass / 1 fail) with the `call` threading removed and green (341) with it.
- Fix for finding 5: `?available_scales` gains a `@details` paragraph naming `hitop_missing_definition` and saying the shipped tables cannot reach it.
- Post-fix re-verification: `devtools::document()` no diff after the regenerated `available_scales.Rd` was committed; `devtools::test()` 14,586 pass / 0 fail / 1 skip; `devtools::check()` 0 errors / 0 warnings / 0 notes; `pkgdown::check_pkgdown()` no problems.
