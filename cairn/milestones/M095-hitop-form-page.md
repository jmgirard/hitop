...
- [ ] AC3: The page shows 15 items on every page but the last and refuses to advance while an item on the current page is unanswered, naming the item's position on the page; a test leaves the first, a middle and the last item of a full page blank in turn, and one item of the last page, reads each refusal, answers the item, and advances.
...
- [ ] T2: `index.html` and `form.js`: fetch the export, check `format`, render instructions, items and options, paginate, require answers.
- [ ] T3: The study link: decode `?c=`, read the descriptor's `items` and `itemOrder`, and `link.html` that encodes a link from a pasted descriptor and study fields.
- [ ] T4: Save: assemble the CSV, download through a Blob and anchor click, show `buildDate` and `packageVersion`.
- [ ] T5: Playwright tests for AC1 and AC2, with the two descriptors written by `write_module()`.
- [ ] T6: Playwright tests for AC3 and AC4; commit the three captured CSVs under `tests/fixtures/`.
- [ ] T7: Playwright tests for AC5 and AC6, serving altered copies of the export locally (LESSONS 2026-08-25 on probing a served copy).
- [ ] T8: README; confirm the Pages deploy serves both pages and the branch's CI run is green.
