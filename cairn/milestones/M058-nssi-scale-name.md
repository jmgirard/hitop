...
**In:** The rename in the keying source `data-raw/hitopsr_items.csv` against a cited
authority (RB tripwire: ip-touching); the four regenerated keying tables
(`hitopsr_items`, `hitopsr_scales`, `hitopsr_subscales`, `hitopsr_definitions`); the
derived `camelCase` stem and therefore the scored output column `hsr_nssi` and its
`_se` sibling; the two rebuilt `inst/extdata/hitopsr_*.docx` artifacts, their
...
citable source printing the scale's full name, and otherwise returns to the plan gate
with the discrepancy recorded as a visible open question, which is what IP1 requires.
Discharged against the HiTOP-SR introduction paper (*Assessment* submission
`ASMNT-26-0390`); its peer-review status, the reconciliation commitment, and why
`hitopsr_definitions`'s matching spelling is no part of the sourcing are recorded in
D-041.

**Out:**
- A `lifecycle` deprecation shim for the old scored column name — the package is
  pre-1.0 and GP2 lets a signature break with NEWS → candidate row if a user is caught.
...
- [x] AC1 The adopted name is sourced, not chosen; which source it is, and why it
      clears D-032's draft bar under peer review and outranks the December 2023
      Workgroup deck under D-018, are settled in the AC6 `DECISIONS.md` entry. The
      string in `data-raw/hitopsr_items.csv` is compared character for character
      against the whole cell in Table 1 of
      `cairn/references/sources/ASMNT-26-0390_Proof_hi.pdf`, whose sha256
      `data-raw/verify_hitopsr_scale_name.R` pins and `cairn/SOURCES.md` records,
      read at verification time from that gitignored shelf, never against a
      transcription held in this repo; a cell carrying a parenthetical stops the
      ...
      source's disagreement with itself and with the deck, per-variant counts from a
      re-runnable search of the PDF rather than copied from this file, and —
      maintainer-attributed, dated — that Appendix A is `hitopsr_definitions`'s own
      text supplied after Tables 1-3 existed. Instrument:
      `data-raw/verify_hitopsr_scale_name.R`.
- [x] AC2 The rename reaches every place the old name lived and moves nothing else.
      A check enumerates the exported datasets programmatically and sweeps every
      character leaf, a list's own names included, plus every tracked file (`git
      ls-files`) opened in its own format rather than as bytes, which is inert
      against gzip `.rda` and zipped `.docx`; it asserts no case-insensitive `nssi`
      outside an allow-list written into the check and no other path — `NEWS.md` and
      `cairn/`, where the name is history; the files whose job is to name it
      (`data-raw/verify_hitopsr_*.R`, `tests/testthat/test-scale-name-hitopsr.R`);
      and `R/data.R` with its generated `man/hitopsr_items.Rd`, carrying D-041's
      also-known-as note. The four keying tables are compared against the same
      objects from a `git worktree` of the merge-base, rows matched by identity,
      identical except cells matching either spelling, blanked on both sides by one
      combined pattern over every column. The renamed row's move is reported, not
      failed, with two assertions: every other row keeps its relative order, checked
      by dropping the renamed row from each side and requiring the key sequences
      identical element for element, since reordering rows by key absorbs any
      permutation (verified by planting one); and its new position equals where the
      adopted name sorts under `sort(method = "radix")` over the merge-base's names
      with the old one replaced, recomputed rather than read off the
      `dplyr::arrange(Scale)` under test. The base-commit reference serves
      invariance only, never certifying a value. Instrument:
      `data-raw/verify_hitopsr_rename.R`, steps 1-2.
- [x] AC3 Scored output renames once and nothing else moves:
      ...
      columns — column set equal, every column identical in value. The renamed
      column's position follows `hitopsr_scales`' order, which AC2 pins, so it is
      reported rather than failed while any reordering of the others fails; its from
      and to positions go in NEWS, positional selection being a break GP2 requires
      be visible. Expected column names are written literally, never re-derived by
      `snakecase::to_any_case()`. Instrument: the same script, step 3.
- [x] AC4 Exactly two files are rebuilt, by `data-raw/artifacts.R` with
      `rebuild_stems <- "hitopsr"` and `rebuild_formats <- "docx"`:
      `hitopsr_US.docx` and `hitopsr_A4.docx`. The REDCap zip is deliberately not
      rebuilt, not being byte-reproducible, and the Qualtrics `.txt` prints no scale
      name. All four artifacts are read back by parsing: both DOCX carry the new
      name and no `NSSI`; `hitopsr_qualtrics.txt` and the REDCap `instrument.csv`
      carry neither name, so their unchanged bytes are a result rather than an
      oversight. Each rebuilt DOCX's `pkgdown/assets/downloads/` copy is
      byte-identical to the file just built, and each rebuilt DOCX's
      `officer::docx_summary()` text, plus its footer read separately, differs from
      the merge-base build only at the scale-name row — in place or moved, that
      table sorting on `Scale` (`R/generate_docx.R:491`) under the running locale —
      and in the footer's `Generated` date, which must have changed.
- [x] AC5 No other distributed file moves: the listings of `inst/extdata/` and
      `pkgdown/assets/downloads/`, taken by listing both directories rather than
      from the manifest, are identical to the merge-base's, and every file in them
      is byte-identical to the merge-base's outside the two rebuilt DOCX and their
      copies, the Qualtrics and REDCap files included. `hitop_artifacts` gains
      exactly two rows, one per rebuilt DOCX, each carrying the rebuild's build date
      and a `changes` note naming the rename, every pre-existing row unchanged — a
      lost row would shrink the sweep, and a rebuild adding none would land against
      D-016 with no version bump.
- [x] AC6 The sign-off is recorded before the keying edit lands:
      `cairn/DECISIONS.md` carries an entry citing the source line AC1 adds and
      naming the adopted name and the scored-column rename, committed no later than
      the commit editing `data-raw/hitopsr_items.csv`. NEWS records the rename as a
      breaking column-name change.
- [x] AC7 `devtools::document()` no diff; `devtools::test()` clean with zero skips
      in `test-artifacts.R` and the HiTOP-SR keying tests, both of which skip
      themselves when Suggests are absent; `devtools::check()` 0 errors, 0 warnings,
      every remaining NOTE listed in this file with a reason.

## Coverage

- AC1 → T1 · AC2 → T2, T3 · AC3 → T2, T4 · AC4 → T5, T6
- AC5 → T7 · AC6 → T1, T8 · AC7 → T9
...
- [x] T5 Rebuild the two DOCX artifacts and their `pkgdown/assets/downloads/` copies
      via `data-raw/artifacts.R` (`rebuild_stems <- "hitopsr"`, `rebuild_formats <-
      "docx"`); confirm exactly two `hitop_artifacts` rows appended and the REDCap
      and Qualtrics files untouched.
- [x] T6 The AC4 parse-backs for all four formats and the bounded DOCX
      text-plus-footer diff.
...
- 2026-08-27: /milestone-review ran. All seven acceptance criteria verified with fresh evidence (recorded in the Review section) and every toolchain consistency check is clean, but the universal consistency gate FAILED: `cairn_validate.py` exits 1 on `weight caps` (this file, 172 plan-owned lines against the <150 cap, shed >=23; heaviest Acceptance criteria 90, Scope 36) and on `iso date format` (line 186, a US-ordered zip member mtime quoted inside a work-log entry). Status returns to `in-progress`; defect return 1 for this milestone. The three-lens review ran and its 13 findings (all from the [O] diff lens; the two [S] lenses found none) are recorded undisposed in the Review section for triage at the next review's gate.
- 2026-08-27: defect return 1 opened on the branch. Both `cairn_validate` failures reproduced first: 172 plan-owned lines (Acceptance criteria 90 · Scope 36 · Tasks 22) and the one non-ISO date. `origin/main` had not moved, so no merge was needed, and nothing in the code half regressed.
- 2026-08-27: the flagged date was converted in place rather than described away. The 2026-08-27 amendment-gate entry quoted the REDCap zip's member mtime as the archive prints it, month-first; it now reads `2026-07-16 22:33`. Only the literal's rendering changed — the same timestamp, the same sentence, the same claim about `zip::zip` storing an mtime — and the validator's date check passes.
- 2026-08-27: gate on the return. Jeff chose the drafted compression over cutting deeper or splitting the file, converting the date literal in place, and fixing three of the review's 13 findings now, the three that are wrong or missing in what ships; the other ten stay undisposed for triage at the next review.
- 2026-08-27: the three findings fixed. (2) `hitopsr_items`'s `@details` claimed "Scale names are those printed in the HiTOP-SR introduction paper's Table 1" for the whole table, which `cairn/SOURCES.md` contradicts — the development workbook is primary for the other 75 names — so the Table 1 attribution now sits on the one scale it belongs to; `document()` regenerated `man/hitopsr_items.Rd`. (12) NEWS gave the position move for the score column and not for its standard-error sibling, which breaks positional selection identically; `verify_hitopsr_rename.R` step 3 now reports both renamed columns rather than the first, and the run gives `hsr_nonSuicidalSelfInjury` 448 to 451 and `hsr_nonSuicidalSelfInjury_se` 524 to 527, which is what NEWS now records. (10) Scope's "Out" promise of a candidate row for renaming any other scale is discharged with a ROADMAP row; search-first found no overlapping row, and the row cross-references M041's AC1 map rather than duplicating it, since that milestone will enumerate the divergences while nothing yet owns deciding whether any is a wrong name. ROADMAP holds at 59 lines and grows to 30,507 bytes, further over the 24,000 budget Jeff accepted the overage on.
- 2026-08-27: all five steps of `verify_hitopsr_rename.R` re-run green after those edits (sweep clean over 289 tracked files, the four tables invariant, every scored column identical in value, both Word forms differing at one row, both distribution listings identical with two files changed); `test-artifacts.R` and `test-scale-name-hitopsr.R` pass 133 with 0 skips. The stale `git worktree` registrations the review found were pruned again after this run; the script defect that leaks them is finding 1, still undisposed.
- 2026-08-27: the amended Scope, Acceptance criteria, Coverage and Tasks went to a fresh-context [O] criteria audit in full mode (user-facing tier) before being written, as an amendment to already-audited wording requires. It returned 10 findings and pronounced AC3, AC6, both of AC2's oracles, its allow-list and blanking clauses, AC4's rebuild filters and parse-back, AC5's manifest anchor, and the In/Out lists sound. Three findings were compression damage of exactly the kind the audit exists to catch, and all three are repaired: AC1 had narrowed its stop condition from any parenthetical to a `(NSSI)` gloss, which would have let a differently glossed cell through and made the criterion narrower than its own instrument; AC2 had replaced the named order procedure with "checked directly", which names no procedure and misdescribed the check besides — the wording the earlier audit put there for exactly this reason is restored; and AC5's "byte-identical" had lost its comparand and the clause naming the two unrebuilt formats, which is what keeps AC4's unchanged-bytes claim non-vacuous. Also applied: AC1's three named obligations on the sign-off entry are restored in place of "the source's standing is settled", AC4's dangling "Its" is re-attached to the built file rather than the pkgdown copy, AC7 says "in this file" again, and Scope's discharged gate condition cites D-041 alone rather than the work log, since no criterion governs a work-log entry.
...
Reviewed 2026-08-27 against branch `m058-nssi-scale-name` at `2256544`, merge-base
`df14f3c`. This is the second review pass; defect return 1 closed the two
`cairn_validate` failures the first pass found. `origin/main` is still at the
merge-base, so no merge was needed. All evidence below is from runs made in this
pass; PR https://github.com/jmgirard/hitop/pull/64.

### Acceptance-criterion evidence


- **AC1 — met.** `Rscript data-raw/verify_hitopsr_scale_name.R` run fresh, exit 0:
  the shelf PDF's sha256 matches the digest pinned in the script and recorded in
  `cairn/SOURCES.md`; the rendering inventory is `Non-suicidal Self-injury` 3
  (pp. 49, 52, 55 — Tables 1-3), `Non-Suicidal Self-injury` 1 (p. 23),
  `Non-Suicidal Self-Injury` 1 (p. 18), plus the reported wrapped occurrence on
  p. 69. The table cell reads `Non-suicidal Self-injury` and matches the string
  committed in `data-raw/hitopsr_items.csv` character for character; the cell
  carries no parenthetical, so the stop condition did not fire. The script reads
  the committed string out of the CSV rather than carrying a copy.
  `cairn/SOURCES.md` carries the "HiTOP-SR scale names" provenance section and a
  numbered OQ-3 in OQ-1's form, its per-variant counts this script's own output
  and its Appendix A note maintainer-attributed and dated.
- **AC2 — met.** `Rscript data-raw/verify_hitopsr_rename.R` run fresh, exit 0.
  Step 1: the sweep runs over 289 tracked files (71 allow-listed) and is clean,
  each format opened in its own terms. Step 2: all four keying tables identical
  to the merge-base outside the renamed cells; every other row keeps its relative
  order, asserted directly; the renamed `hitopsr_scales` row moves 43 to 46,
  equal to where the adopted name sorts under a recomputed
  `sort(method = "radix")` over the merge-base's names. The check's allow-list
  matches AC2's enumeration.
- **AC3 — met.** Step 3 of the same run: against the merge-base build,
  `score_hitopsr(sim_hitopsr, items = 1:405, calc_se = TRUE)` returns the same
  column set once the two columns are renamed, and every column is identical in
  value. Both renamed columns' moves are reported —
  `hsr_nonSuicidalSelfInjury` 448 to 451 and `hsr_nonSuicidalSelfInjury_se`
  524 to 527 — following `hitopsr_scales`' order; no other column reorders.
  NEWS records both position moves.
- **AC4 — met.** Exactly the two DOCX were rebuilt. Verified independently of the
  verifier: each `pkgdown/assets/downloads/` copy is byte-identical to its
  `inst/extdata/` file (sha256 `c6f2b537…` US, `2b586012…` A4); both DOCX carry
  `Non-suicidal Self-injury` once and `NSSI` zero times; `hitopsr_qualtrics.txt`
  and the REDCap `instrument.csv`, unzipped and read, carry zero occurrences of
  either spelling, so their unchanged bytes are a result. Step 4 of the verifier:
  each Word form's `officer::docx_summary()` text differs from the merge-base
  build in exactly one paragraph (row 2064, `NSSI` to `Non-suicidal Self-injury`),
  and the separately read footer is asserted to have changed.
- **AC5 — met.** Step 5: `inst/extdata/` and `pkgdown/assets/downloads/` each
  hold 24 files, both listings identical to the merge-base's, and in each
  directory exactly the two DOCX differ. `hitop_artifacts`: 33 rows unchanged,
  exactly 2 added, one per rebuilt DOCX, carrying the `changes` note naming the
  rename.
- **AC6 — met.** `cairn/DECISIONS.md` D-041 landed in `93106a8`; the keying edit
  to `data-raw/hitopsr_items.csv` landed in `891e7f1`, the next commit, so the
  sign-off precedes the edit. D-041 cites the `SOURCES.md` provenance section and
  OQ-3 and names the adopted string and the `hsr_nssi` to
  `hsr_nonSuicidalSelfInjury` rename. NEWS records the rename as breaking, naming
  both old and new column names.
- **AC7 — met.** `devtools::document()` leaves the tree clean (`git status` shows
  only this milestone file). `devtools::test()`: FAIL 0 | WARN 0 | SKIP 1 | PASS
  14598; the single skip is `test-keying.R:102`, the pre-existing OQ-1 PID-5
  placeholder this milestone does not touch — `test-artifacts.R` (121 pass) and
  `test-scale-name-hitopsr.R` (12 pass) each ran directly at 0 skips.
  `devtools::check()`: Status OK, 0 errors, 0 warnings, 0 notes, so AC7's
  NOTE-listing obligation is empty.

### Consistency gate — passed

`cairn_validate.py` exits 0; every check passes, including the two that failed
the first pass (`weight caps` — the plan-owned body is now under the cap — and
`iso date format`). Advisories only: 24 dangling id tokens, all pre-existing
D-001–D-012 legacy references, four of them this file's own quotations of them.
`cairn_impact.py` was skipped: `cairn/DESIGN.md` is not in the diff, so no
principle changed.
...
"No problems found"; NEWS carries the user-visible entry with no milestone
number; no new top-level files; `devtools::check()` Status OK.

### Independent review — three lenses

Full three-lens fan-out (the diff touches executable surface), all fresh-context.
Findings from the first pass that were fixed on the branch are not repeated here;
findings that pass carried forward are marked *carried*.

- **[S] prior-review-record: no findings.** Archived `## Review` sections and
  `cairn/LESSONS.md` are clean against this diff; RR04's holdings (naming
  authority, the mechanically derived stem, the also-known-as pattern, the
  DOCX-only rebuild) are all honored, and its fallback string was superseded by
  the paper it named as outranking the deck. The probe
  `gh api repos/jmgirard/hitop/pulls/comments?per_page=1` returned `[]`, so the
  per-PR walk was skipped.
- **[S] blame-history: no new findings.** Confirmed `7e545ba` added
  `definition_scale_labels` solely to reconcile the definitions spelling against
  the then-abbreviated scale table, so deleting it drops no other guard, and that
  the `stopifnot` beside it is untouched. Independently re-derived three carried
  findings and sharpened one by execution: `snakecase::to_any_case()` maps all
  three casings of the name to one stem.
- **[O] diff-bug: 14 findings**, ranked as the reviewer ranked them; two are new
  this pass. Findings 1, 2 and 4 were re-verified in this session against the
  implementation rather than taken on the reviewer's word.
  1. `data-raw/verify_hitopsr_rename.R:147` leaks a `git worktree` on every run:
     `on.exit()` never fires at top level under `Rscript`, and the `dir.exists()`
     guard cannot fire because `tempdir()` differs each run. **Verified** — two
     stale worktrees were registered after this pass's runs; pruned again here.
     *Carried from pass 1.*
  2. **New.** `R/available_scales.R:55-57` and
     `tests/testthat/test-available_scales.R:125-129` both still say "the two
     tables disagree on one HiTOP-SR label, so a name join would drop that
     scale". **Verified false as of this branch**: `hitopsr_scales$Scale` and
     `hitopsr_definitions$Scale` are now setequal over all 76. The test comment
     names this milestone's own case ("if a later milestone reconciles the
     labels, this test says so out loud") but the body never asserted the
     disagreement, so it passes silently. AC2's sweep cannot see it — the prose
     describes the old state without spelling `nssi`.
  3. `data-raw/hitopsr_info.R:47-52`'s replacement comment claims the
     `stopifnot` "is what catches a genuine drift"; it compares stems, and every
     casing of the name maps to one stem, so the casing divergence D-041's
     reconciliation clause plans for would pass silently. *Carried from pass 1.*
  4. **New.** `cairn/SOURCES.md:392-393` — the branch reworded the OQ preamble
     from "Both" to "OQ-1 and OQ-2 are encoded as `skip()`-ed tests … Neither has
     been resolved", **verified** against OQ-2's own heading at line 408, which
     reads `[RESOLVED 2026-07-09]`, and against the suite, where OQ-1's is the
     only skip. Pre-existing in substance; the branch sharpened it by naming both.
  5. AC4's footer clause is under-verified: the check asserts only that the two
     footers differ, so any other footer change would pass as "the stamp moved".
  6. AC5's per-row manifest requirements are printed, not asserted: step 5 `cat`s
     the `changes` string and never inspects `build_date` or the note's content.
     (`test-artifacts.R:53-66` covers the date separately.) *Carried from pass 1.*
  7. AC4's "carry neither name" is half-verified for the two unrebuilt formats:
     the sweep proves no `nssi`, nothing asserts no `Non-suicidal Self-injury`.
     *Carried from pass 1.* (Checked by hand at AC4 above: both are zero.)
  8. Step 2's position oracle assumes unique `Scale` values; on `hitopsr_items`
     (6 matches) and `hitopsr_definitions` (2) it would fail spuriously.
     Unreached, since neither table's rows move. *Carried from pass 1.*
  9. NEWS records only the scoring break: `hitop_module("hitopsr", "NSSI")` now
     aborts and `read_module()` rejects any descriptor recording `"NSSI"`.
     Mitigating: modules ship first in this same unreleased 0.2.0.
     *Carried from pass 1.*
  10. `blank_renamed()` blanks a whole cell on substring match, so a real change
      co-located with the name in one cell would be invisible. No such cell
      exists today. *Carried from pass 1.*
  11. `verify_hitopsr_scale_name.R:35` identifies the committed name by grepping
      item text, coupling the source check to wording IP1 could change; guarded
      by a `length == 1` `stopifnot`, so it fails loudly. *Carried from pass 1.*
  12. The also-known-as note sits only on `?hitopsr_items`; a user holding
      `hsr_nssi` would open `?score_hitopsr`, which says nothing.
  13. `NEWS.md:6-8` can be read as attributing all 76 names to the introduction
      paper — the same ambiguity the `R/data.R` fix removed in pass 1.
  14. `NEWS.md:12-13` has a ragged mid-paragraph line break left by the `_se`
      edit. Cosmetic.

  Clauses the [O] lens checked and pronounced sound: AC1's instrument (sha256
  agreement, the now-reachable glossed-cell branch, the non-circular comparison);
  AC2's per-format sweep and allow-list; AC3's literal rename map and reported
  moves, its NEWS figures confirmed against a live run; the keying edit (exactly
  six `Scale` cells, no other column or item text moved) and the
  behavior-preserving removal of `definition_scale_labels`; AC6's commit
  ordering; AC5 against D-016 (two rows, `instrument_version` untouched, the
  pkgdown byte-identity covered by `test-artifacts.R:195-211`); and
  `artifacts.R`'s left-in-place rebuild filters, which the file's own header
  prescribes.

### Return floor

No finding demonstrates an acceptance criterion failing inside its named
procedure's domain: findings 5-8 and 10 are limits of the criteria's own
instruments, reported by the reviewer rather than shown to have let a defect
