...
- 2026-09-23: step-7 approval: m114-form-item-card-layout approved for merge, in hitop-form and hitop, with four fix-now edits (F2, F3, F6, F8) and a WebKit run of the layout spec.
...
- Triage, Jeff's choice at the gate. Fixed now in hitop-form `1fa2282`: F2 (Y2 paired the i-th card with the i-th export item, now looked up by `data-number`), F3 (the options' top is asserted at or below the legend's bottom), F6 (the comments in `index.html` and the spec now say the top border meets the legend at its vertical middle), F8 (each Y1 page asserts `documentElement.scrollWidth <= innerWidth`, green today). Chromium layout spec after the fixes: 12 passed.
- F1, the browsers: Jeff chose the cross-browser run. WebKit build 2215 ran the layout spec, 12 passed. Firefox build 1495 never launched on this Mac (macOS 27): a graphics compositor crash at start, in and out of the command sandbox, so no Firefox measurement exists. Chromium and WebKit are the browsers measured. The Playwright installer's extraction step hung under the command sandbox, so both archives were unzipped into the cache by hand.
- Noted, no change: F5 (the gap between number and text grew by about one space width), F10 (Y3 checks the light colour scheme only), F11 (`scrollWidth` and `clientWidth` are whole pixels).
- Rejected: F4 (`clear: both` is a deliberate guard, a style point), F7 (each README claim is backed by an assertion, confirmed by the prior-review lens), F9 (Y1's two forms are AC1 as planned), F12 (the page sets a numeric line height), F13 (the page is English only).
