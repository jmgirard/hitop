...
- 2026-08-22: verify slot clean after the review fixes — `devtools::test()` FAIL 0 / WARN 0 / SKIP 1 / PASS 13778; `devtools::check()` Status OK (0/0/0); `devtools::document()` no diff. Status back to review.
- 2026-08-22: review fixes — all ten findings addressed. Code: the crosswalk moved out of the `include_scoring` block (a shuffled form with no key was unscoreable), gained a `!is.null(module)` gate per the gate decision (a 405-pair paragraph on a participant page is outside D-036's sign-off), and `item_order` is now `as.integer()`. Tests: the AC5 per-scale loop reads each scale's printed numbers from `docx_scoring_rows()` instead of deriving them by `match()` against `item_order`; mutation-checked by re-pointing `printed_of()` at `sort(item_order)`, which now fails the loop on all five scales where it previously passed green. Added tests for the no-key crosswalk, the no-module suppression, and the integer type; `withr::local_seed()` replaces bare `set.seed()`, the missing `skip_if_no_docx()` guard added, and the two bare `"(R)"` greps anchored to a digit per the M26 lesson. Docs: `?generate_docx_hitopsr` and the vignette now state when the crosswalk is and is not printed and how to reorder collected columns before scoring; `NEWS.md` gained the positional-signature note and its cross-reference direction was corrected. The documented reorder idiom was checked by execution before it was written — the first draft, `order(order(item_order))`, was the inverse permutation and wrong; `order(item_order)` is the verified form.
