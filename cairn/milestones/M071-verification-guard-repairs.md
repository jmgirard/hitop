...
- [ ] T5: Assert `pkgdown/assets/downloads/` exists when the suite runs from a git checkout, covering both `test-scale-name-hitopsr.R:342-352` and `test-artifacts.R:197`; prove it red by renaming the directory and green with it present.
- [ ] T6: Point `verify_hitopsr_names.R:166-178`'s control at `hitopsr_table1_watermark_fragments`; delete the hardcoded copy.
