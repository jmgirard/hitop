...
- 2026-08-30: gate directed the fix-now finding. `git_ok()` in `tests/testthat/helper-merge-base.R` now tests `file.exists(.git)`, so AC4's floor and M059's merge-base checks hold in a linked worktree and still skip under `R CMD check`; `devtools::test()` 0 failures / 16152 passing. Gate also directed the four follow-up findings to one ROADMAP candidate row.
