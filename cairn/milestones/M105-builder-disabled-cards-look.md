...
Fresh probes, 2026-09-21: a scratch Playwright spec (full Chromium, headless) served builder branch b2b4f54. It ran a Word build under light and a Qualtrics build under dark, with all scales ticked. The first dark run read the cards after the build ended (status "Ready."). The probe now holds each webR worker message for 1.5s during the read. It also asserts a status that starts with "Building". The hold does not change the page code or CSS. Every read below landed during the build.

- AC1: all 6 card reads (3 cards, 2 schemes) passed. Background and `border-top-color` equal `#downloadBtn`'s, the border is `dashed`, and `box-shadow` is `none`. Both text colours equal the `--disabled-fg` probe. Control: the same probe on main's `index.html` (c56a9fb) failed all 6 reads.
- AC2: in both runs, the current card (Word in light, Qualtrics in dark) had an `::after` background equal to the `--accent` probe and `::before` content `""`. The other 2 cards had an `::after` background equal to the `--bg` probe and `::before` content `none`.
