...
Sync: branch contains `origin/main` (e7c00c2e); no merge needed. Suite run 2026-09-22 on b8610437: `devtools::test()` 0 failures, 0 errors, 17878 passes, 13 skips.

- AC1: `test-generator-descriptor.R` "generate_redcap_hitopsr() writes the dictionary's item field names" (12 passes) parses the ZIP's dictionary, keeps `radio` rows, asserts no instructions field among them, and compares `columns` to those names and to `sprintf("hsr_%03d")` names, for the full instrument and two scales. "generate_qualtrics_hitopsr() writes the file's question IDs" (24 passes) does the same from the `[[ID:]]` tokens of `[[Question:MC:SingleAnswer]]` blocks (helper `read_qualtrics()`), under the default and `study2` prefixes. Both help pages (`R/generate_redcap.R:84`, `R/generate_qualtrics.R:83`) call the names the ones the generated file assigns, say an export with item variable names as headers uses them, and say to pass `items` for other names. The REDCap help gives one example (a renamed field). The Qualtrics help gives two (another header setting, a renamed question). Source comments at `R/generate_redcap.R:110` and `R/generate_qualtrics.R:110` cite the two notes. The note `qualtrics2026exportheader.md:25` states the one-module, default-prefix scope.
- AC2: Four tests in `test-generator-descriptor.R` pass. A module read from a REDCap descriptor goes to the Word generator, which writes no `columns` field, and reads back with no attribute (4 passes). The same carried module goes to the Qualtrics generator with `study2`, which writes `study2_NNN` names (3 passes). A REDCap sidecar replaces planted `other_N` names with its own (2 passes). `write_module()` on a module with no attribute writes no field (2 passes). Each of these tests asserts `format` `"1.0"`.
- AC3: Three tests in `test-module_file.R` pass. A file with the field reads to a character attribute. `write_module()` writes it back as a JSON array (`"columns": [`) with the same value (4 passes). A file with no field and a file with `null` read with no attribute. The `null` control reads its items in full (3 passes). A bare string `"hsr_066"` raises `hitop_module_file_bad_columns` with "not 1." in its message, after `expect_gt(min(lengths(hitopsr_scales$itemNumbers)), 1L)` (3 passes).
- AC4: "read_module() refuses a bad `columns` field, naming the file" (23 passes) runs 11 cases. The value is an object or a number. An element is a number, boolean, `null`, `""`, array or object. The length is short or long. A name repeats. Each asserts `hitop_module_file_bad_columns` and the file path in the message, with a passing control. "write_module() refuses a bad `columns` attribute" (16 passes) runs 5 cases: not character, `NA`, `""`, wrong length, and a repeated name. Each asserts an `rlang_error` whose message names `columns` and that no file exists, with a passing control.
- AC5: `test-module-columns-scoring.R` passes 4 tests. Both functions with `items` omitted and with `items = NULL` give results identical to the same call with the names passed (5 passes). A supplied `items` wins over columns that name nothing in the data (2 passes). No module, a module with no attribute, and `layout = "printed"` each abort in both functions with a message holding "items" and "Pass" (18 passes). The end-to-end test generates a REDCap module ZIP and descriptor and names simulated data by the parsed dictionary. Scores and reliability from `read_module()` alone equal those with the parsed names as `items` (3 passes).
