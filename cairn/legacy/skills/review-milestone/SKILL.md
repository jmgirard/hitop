...
6. **If everything passes — get merge authorization, then close out:**
   ...
   - **Only after the PR is actually merged** (by you or by Jeff), on up-to-date `main`: verify every acceptance criterion passes, then set Status → `DONE` and move the entry to Completed, condensed to the paragraph form in tracking-rules.md — a one-paragraph outcome (what shipped, why, PR link), **without** the acceptance-criteria checklist or task list (their verification is captured in the LOG entry).
   - Append a LOG.md entry (what shipped, verification results, follow-ups).
   - **Inline doc sync:** remove now-fixed items from DESIGN.md Known issues; update the affected DESIGN.md sections if the architecture changed; update ROADMAP.md if this completed a phase outcome (and advance the "(current)" phase marker if the phase is finished); update CLAUDE.md Commands/Conventions if tooling changed (e.g., remove the "activates at M001" caveat once tests exist).
