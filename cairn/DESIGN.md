...
8. **A shuffled whole-instrument Word form from the browser builder records its printed order nowhere** — ticking every scale makes the builder page pass no `module` (M049), so `generate_docx_hitopsr()` prints no crosswalk, which it declines for a full instrument because 405 pairs would fill a participant-facing page. The generator returns the order in the `item_order` attribute and the page discards it, so a researcher who fields that form and enters responses in printed order gets wrong scale scores from `score_hitopsr()` with no error raised. The page's on-screen notice says so and tells the visitor to keep their own record; giving them a record to keep is unresolved. (Accepted at the M049 review gate, 2026-08-24.)
