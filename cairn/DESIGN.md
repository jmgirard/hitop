...
8. **The browser builder discards a shuffled whole-instrument Word form's printed order** — ticking every scale makes the builder page pass no `module` (M049), so `generate_docx_hitopsr()` prints no crosswalk, which it declines for a full instrument because 405 pairs would fill a participant-facing page. The package side is closed: passing `descriptor =` writes the printed order into a module descriptor, which `read_module()` returns on the `item_order` attribute (M055). The page passes no `descriptor` and offers no such download, so it still discards the order, and its on-screen notice still tells the visitor to keep a record it does not give them. M056 carries the app change. (Narrowed to the app remainder at M055, 2026-08-24; accepted at the M049 review gate, 2026-08-24.)
