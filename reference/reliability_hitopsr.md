...
- items:
  ...
  attribute. Each column must be numeric or logical, or character
  holding only numbers (blank cells and `NA` values count as missing,
  but the text `"NA"` is refused). A haven labelled column is read as
  its plain values. Any other column, such as the choice text of an
  online export or a factor, is an error of class
