...
- prefix:
  ...
  Columns that carry the prefix and a number that is not one of those
  expected names are not labelled, and a warning of class
  `hitop_unpadded_items` names them, in a sentence per kind: a number
  without that padding is reported as not zero-padded to three digits,
  and a number outside 1 to 405 is reported as out of range, whatever
  its padding. That warning is raised whether or not any other column
