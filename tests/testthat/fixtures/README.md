...
| `responses-pid5.csv`, `responses-pid5sf.csv`, `responses-pid5bf.csv` | Copied from `tests/fixtures/` in the hitop-form repository at commit `bfb1d9c` (2026-09-20). Generated there by the same command, `WRITE_FIXTURES=1 npx playwright test tests/save.spec.js`. Each is one PID-5 form (full, short, brief) the page saved for study `fixture` and participant `p001`. The same pattern answers every item, and on these forms, whose options are worth 0 to 3, it writes 3, 2, 1, 0 repeating down the form. Stored as LF, as above. |
