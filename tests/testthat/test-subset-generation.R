# No-regression and round-trip invariants for the `subset` argument (M24).
#
# Two properties, checked across all three HiTOP-SR generators:
#   (1) subset = NULL produces the same artifact as omitting the argument;
#   (2) a subset naming ALL 76 scales reproduces the full instrument.
# (2) is the stronger check: it exercises the subset code path end to end and
...
all_sr_scales <- function() hitop_subset("hitopsr", hitopsr_scales$camelCase)

test_that("Qualtrics: subset = NULL and an all-scales subset match the default", {
  ...
  suppressMessages(generate_qualtrics_hitopsr(file = f_null, subset = NULL))
  suppressMessages(generate_qualtrics_hitopsr(file = f_all, subset = all_sr_scales()))
...
test_that("REDCap: subset = NULL and an all-scales subset match the default", {
  skip_if_no_zip()
  ...
  suppressMessages(generate_redcap_hitopsr(file = f_null, subset = NULL))
  suppressMessages(generate_redcap_hitopsr(file = f_all, subset = all_sr_scales()))
...
test_that("DOCX: subset = NULL and an all-scales subset match the default", {
  skip_if_no_docx()
  ...
  suppressMessages(generate_docx_hitopsr(file = f_null, subset = NULL))
  suppressMessages(generate_docx_hitopsr(file = f_all, subset = all_sr_scales()))
...
test_that("an all-scales subset resolves to the whole instrument", {
  s <- all_sr_scales()
