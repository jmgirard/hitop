...
# --- subset scoring (M37) ---------------------------------------------------
# The subset path hands score_engine() the same three inputs as the full path,
...
test_that("score_hitopsr(subset=) returns exactly the subset's scales, in row order", {
  s <- sub_four()
  out <- score_hitopsr(
    sub_data(s), items = seq_len(s$nItems), subset = s, append = FALSE
  )
...
test_that("score_hitopsr(subset=) equals the full run, both missing modes, NAs or not", {
  s <- sub_four()

  # Injected NAs must land inside the subset's own columns to separate
  # "available" from "complete" on the subset path at all. One item per scale,
  ...
  for (dat in list(sim_hitopsr, holed)) {
    for (mode in c("available", "complete")) {
      ...
      part <- score_hitopsr(
        ...
        subset = s, missing = mode, append = FALSE
      )
...
test_that("score_hitopsr(subset=) reverse-keys HSR 310 at its subset position", {
...
test_that("score_hitopsr(subset=) returns standard errors for just its scales", {
  s <- sub_four()
  full <- score_hitopsr(sim_hitopsr, items = 1:405, calc_se = TRUE, append = FALSE)
  part <- score_hitopsr(
    sub_data(s), items = seq_len(s$nItems),
    subset = s, calc_se = TRUE, append = FALSE
  )
...
test_that("score_hitopsr(subset=) takes column names as well as positions", {
  s <- sub_four()
  dat <- sub_data(s)
  by_pos <- score_hitopsr(dat, items = seq_len(s$nItems), subset = s, append = FALSE)
  by_name <- score_hitopsr(dat, items = names(dat), subset = s, append = FALSE)
